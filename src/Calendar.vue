        ...
        <div v-show="isOpen" class="calendar-container">
            ...
            <div v-if="currentDay" class="time-selector">
                <div class="hours">
                    ...
                    <button :class="['time-button', {active: currentHour === hour,}]" v-for="hour in availableHours"
                            :key="'hour-' + hour" @click="selectHour(hour)">
                        {{ hour }}
                    </button>
                </div>
                <div v-if="currentHour" class="minutes">
                    <p class="title-date-hour">Выберите минуты:</p>
                    <button class="time-button" v-for="minute in availableMinutes" :key="'minute-' + minute"
...
export default {
    ...
    computed: {
        ...
        availableHours() {
            return Array.from({length: this.durationWorkDay}, (_, i) => i + this.startHour);
        },
        availableMinutes() {
            return ['00', 15, 30, 45];
        },
